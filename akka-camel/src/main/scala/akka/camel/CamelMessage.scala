...
import akka.japi.{ Util, Function ⇒ JFunction }
...
case class CamelMessage(body: Any, headers: Map[String, Any]) {
   ...
   */
  def bodyAs[T](implicit t: Manifest[T], camelContext: CamelContext): T = getBodyAs(t.erasure.asInstanceOf[Class[T]], camelContext)
   ...
   * using the `getCamelContext` method, and is available on the [[akka.camel.CamelExtension]].
   */
  def withBodyAs[T](implicit t: Manifest[T], camelContext: CamelContext): CamelMessage = withBodyAs(t.erasure.asInstanceOf[Class[T]])
   ...
   *
   */
  def headerAs[T](name: String)(implicit t: Manifest[T], camelContext: CamelContext): Option[T] = header(name).map(camelContext.getTypeConverter.mandatoryConvertTo[T](t.erasure.asInstanceOf[Class[T]], _))
   ...
   * Java API
   */
  def getHeaderAs[T](name: String, clazz: Class[T], camelContext: CamelContext): T = headerAs[T](name)(Util.manifest(clazz), camelContext).get
